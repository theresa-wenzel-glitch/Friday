...
(function () {
  ...
  // Das Brett ist die von Theresa gelieferte Insel-Illustration
  // (werkzeuge/artwork/inselkarte.jpg, als window.INSELBILD eingebettet) -
  // kein prozedural gezeichnetes Gelände mehr. Jede der 19 Stationen liegt
  // auf ihren festen Bildkoordinaten (f.bx, f.by aus daten.py); die
  // Leinwand hat exakt die Pixelmaße dieses Bildes.
  const BREITE = D.bild.breite, HOEHE = D.bild.hoehe;

  function feldPunkt(f) { return [f.bx, f.by]; }
  ...
  function renderBrettBereich(zustand, sitz) {
    ...
    // Das Brett selbst: Theresas Insel-Illustration als Hintergrund, keine
    // prozedural gezeichnete Landschaft mehr.
    svg += '<image href="' + window.INSELBILD + '" x="0" y="0" width="' + BREITE + '" height="' + HOEHE +
      '" preserveAspectRatio="xMidYMid slice"/>';

    D.felder.forEach(function (f) {
      const [cx, cy] = feldPunkt(f);
      const anklickbar = !!zielFelder[f.id];
      svg += '<g class="hexfeld' + (anklickbar ? " anklickbar" : "") + '" data-feld="' + f.id + '">';
      // Der Klickbereich ist unsichtbar, solange das Feld kein gültiges
      // Zug-Ziel ist - das Bild soll für sich sprechen. Nur bei einem
      // gültigen Ziel liegt eine sanft pulsierende Hervorhebung darüber.
      const radius = f.typ === "grruine" ? 58 : 46;
      if (anklickbar) {
        svg += '<circle class="stationsring" cx="' + cx + '" cy="' + cy + '" r="' + radius +
          '" fill="var(--akzent-hell)" fill-opacity="0.4" stroke="var(--gut)" stroke-width="4"/>';
      } else {
        svg += '<circle cx="' + cx + '" cy="' + cy + '" r="' + radius + '" fill="transparent"/>';
      }

      if (M.hilfen.istUntersuchbar(zustand, f.id)) {
        // Unten am Feld statt oben rechts - das Bild hat an manchen Ruinen
        // dort schon eine eigene, rein dekorative Zahl eingezeichnet, und
        // die beiden sollen nicht wie ein doppeltes Abzeichen wirken.
        const by2 = cy + radius + 18;
        svg += '<circle cx="' + cx + '" cy="' + by2 + '" r="15" fill="var(--blatt)" ' +
          'stroke="var(--tinte2)" stroke-width="2" stroke-dasharray="4 3"/>';
        svg += '<text x="' + cx + '" y="' + (by2 + 5) + '" font-size="13" font-weight="800" text-anchor="middle" fill="var(--tinte2)">' +
