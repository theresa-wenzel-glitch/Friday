 ...
 * Dadurch muessen Mobile, Web und Admin nicht pro Endpunkt raten, wie eine
 * Antwort aussieht.
 */
...
export interface ApiErrorBody {
  ...
  code: ApiErrorCode;
  /** Fuer Menschen lesbare Meldung (deutsch). */
  message: string;
...
export interface Page<T> {
  items: T[];
  /** Gesamtzahl der Treffer ohne Beruecksichtigung von limit/offset. */
  total: number;
  limit: number;
  offset: number;
}
...
/** ISO-8601-Zeitstempel als String, so wie er ueber die API geht. */
